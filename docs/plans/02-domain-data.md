...
- Apps / Consumer Keys / Consumer Key access mode（`all | restricted`）。
- Upstream keys / endpoints / quotas / counters。
- Provider presets：内置 preset 由 `@manageyourllm/shared` 提供；本地自定义 preset 存入 `provider_presets`，永不存 secret。
- Target names（`name` 为主键） / Public Models / Candidates / Model Groups / Members。
- Sticky bindings / Sticky sessions。
- Circuit breakers / endpoint health。
- Usage records / request trace logs / debug content logs / audit events / daily consumption stats。
- Cost ledger：pricing entries / plans（`token | coding`）。
- Model reference entries / sync status。
- Backups metadata。
- Settings（`admin_settings` 单例）。
...
实际实现时按聚合边界合并了路由状态与可观测性 repository，最终文件结构如下：
...
infrastructure/db/repositories/
  ...
  routing-state.repository.ts    # 合并 sticky / breaker / endpoint health
...
说明：
- `provider-preset.repository` 只持久化本地自定义 preset；内置 preset 继续由 `@manageyourllm/shared` 提供。
- `target_names` 使用 `name` 作为主键，公共模型与模型组共享同一命名空间。
- `upstream_keys.auth_type` 简化为 `pat | oauth`；具体 OAuth 策略细节后移。
- `plans` 表合并原 `token_plans` / `coding_plans`，通过 `plan_type` 区分。
