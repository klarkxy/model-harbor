...
test.describe('daily use', () => {
  ...
  test.beforeAll(async () => {
    ...
    const appName = `Daily Use App ${suffix}`;
    ...
    await postJson(adminCtx, '/api/admin/public-models', {
      ...
      candidates: [{ upstreamKeyId: upstream.data.id, realModelName: 'gpt-4o-real' }],
    });

    const consumer = await postJson<ApiEnvelope<{ consumerKey: { id: string }; rawKey: string }>>(
      adminCtx,
    ...
    const consumerRawKey = consumer.data.rawKey;

    await postJson(adminCtx, '/api/admin/pricing', {
      providerType: 'openai_compatible',
      upstreamKeyId: upstream.data.id,
    ...
    await postJson(adminCtx, '/api/admin/plans', {
      planType: 'token',
      name: planName,
      providerType: 'openai_compatible',
      upstreamKeyId: upstream.data.id,
  ...
  test('admin web shows usage, traces and plan reminders', async ({ page }) => {
    ...
    await page.getByRole('cell', { name: modelName }).first().click();
    await expect(page.locator('text=gpt-4o-real').first()).toBeVisible();

    await page.goto(`${WEB_URL}/plans`, { waitUntil: 'networkidle' });
