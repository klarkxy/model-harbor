...
describe('ModelReferenceService', () => {
  ...
  it('recommends public model drafts and detects name conflicts', async () => {
    ...
    const draft = await service.recommendDraft({
      entryIds,
      providerAccountId: 'uk_1',
      createGroup: true,
    });
    expect(draft.models).toHaveLength(2);
    expect(draft.models[0]!.nameConflict || draft.models[1]!.nameConflict).toBe(true);
    expect(draft.conflicts.length).toBeGreaterThan(0);
    expect(draft.channel).toBeDefined();
    expect(draft.channel!.members).toHaveLength(2);
  });
});
