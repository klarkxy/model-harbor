...
describe('backup service real restore', () => {
  ...
  it('restores database file from a full backup', async () => {
    ...
    // 在备份之后写入一条新数据。
    await new ClientRepository(testDb.db).createClient({ name: 'after-backup', enabled: true });
    const appsBeforeRestore = await new ClientRepository(testDb.db).listClients();
    ...
    try {
      const appsAfterRestore = await new ClientRepository(newDb).listClients();
