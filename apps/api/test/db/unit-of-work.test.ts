...
import { clients } from '../../src/infrastructure/db/schema.js';
...
describe('unit of work', () => {
  ...
  it('commits successful transaction', async () => {
    ...
    const rows = await testDb.db.select().from(clients).where(eq(clients.id, 'app_1'));
  ...
  it('rolls back failed transaction', async () => {
    ...
    const rows = await testDb.db.select().from(clients).where(eq(clients.id, 'app_2'));
