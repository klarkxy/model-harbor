...
export async function setupClientKey(): Promise<SetupClientKeyResponse['data']> {
  const res = await api.post<SetupClientKeyResponse>('/api/admin/setup/client-key');
  return res.data;
}
