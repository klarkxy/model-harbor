...
import { createModel } from '../api/admin/models.js';
import { createChannel } from '../api/admin/channels.js';
...
const providerAccountOptions = ref<Array<{ label: string; value: string }>>([]);
...
async function load() {
  ...
  try {
    ...
    providerAccountOptions.value = keys.map((k) => ({ label: k.name, value: k.id }));
  } catch (err) {
...
function onRecommend() {
  ...
  recommendForm.value = { providerAccountId: '', createGroup: false, groupName: '' };
...
async function onGenerateDraft() {
  ...
  try {
    recommendDraft.value = await recommendModelReferenceDraft({
      ...
      providerAccountId: recommendForm.value.providerAccountId || undefined,
...
async function onApplyDraft() {
  ...
  try {
    ...
    for (const pm of recommendDraft.value.models) {
    ...
    if (recommendDraft.value.channel && !recommendDraft.value.channel.nameConflict) {
      const members = recommendDraft.value.channel.members
        .map((m) => {
          const modelId = created[m.modelName];
      ...
      if (members.length > 0) {
        await createChannel({
          name: recommendDraft.value.channel.name,
          displayName: recommendDraft.value.channel.displayName,
          description: recommendDraft.value.channel.description,
          enabled: true,
          members,
        });
      }
  ...
  <NSpin :show="loading">
    ...
    >
      <NForm label-placement="left" label-width="140px">
        <NFormItem :label="t('modelReference.providerAccount')">
          <NSelect
            v-model:value="recommendForm.providerAccountId"
      ...
      <div v-if="recommendDraft" style="margin-top: 16px">
        ...
        <NCard size="small" :title="t('modelReference.draftModels')">
          <ul>
            <li v-for="pm in recommendDraft.models" :key="pm.name">
              {{ pm.displayName }}
        ...
        <NCard
          v-if="recommendDraft.channel"
          size="small"
          :title="t('modelReference.draftChannel')"
          style="margin-top: 12px"
        >
          {{ recommendDraft.channel.displayName }}
          <NText v-if="recommendDraft.channel.nameConflict" type="error">
